...
#创建一个文件handler
handler=logging.FileHandler('weibo3.log')
...
def  readtxt(topicnumber):
    topicN="number"+str(topicnumber)
    file_url="/home/bewolf/document//weibospider/topic/"+str(topicN)+".txt"
...
def main():
    ...
    totaltextnumber=4
    for i in range(1,totaltextnumber+1):         #由于range的问题，所以需要的toaltextnumer必须加1
            topicnumber=i            #文本都是number1,等等
            topic_text="number"+str(topicnumber)
            url_list=[]
            url_list=readtxt(topicnumber)       #从numberx.txt读取到url列表放到url_list
            url_list_len=len(url_list)                 #判断url_list的长度，在这个长度下，循环读取url并获取详细的细节
            #如果程序从中间某个网页终端，可以从下一个url开始继续爬取，这个是起始的数目，要注意的是，这个数值是比在url_list的序号是大一的,所以真正要开始爬取的行数要减去1.赋值给startnumber
            for  j in range(0,url_list_len):
                    url_time=str(url_list[j])
                    driver.get(url_time)
                    #减少超时造成的影响
                    ...
                    topictime=url_time[-33:-8]                   #-29：-8是没有小时的，有些网页需要从小时开始抓取，因此，-33:-8
                    filename="/home/bewolf/document/weibospider/data/"+str(topic_text)+"/"+str(topictime)+".json"     
                    for k in range(1,page_number+1):
                            logger.info("加载第"+str(k)+"页")
